      ...
      <TableBody>
        {vaults.map((vault) => (
          <TableRow key={vault.symbol}>
            ...
            <StateCell state={vault.state} />
            <TableCell>{vault.startDate}</TableCell>
            <LiquidityCell liquidity={vault.liquidity} />
            <TableCell>
...
function VaultCell({ vault }: { vault: Vault }) {
  return (
...
function StateCell({ state }: { state: Vault["state"] }) {
  return (
    <TableCell>
      <span
        className={`inline-flex items-center rounded-full px-2 py-1 text-xs ${
          state === "Fixed" ? "bg-yellow-500/10 text-yellow-500" : "bg-green-500/10 text-green-500"
        }`}
      >
        {state}
      </span>
...
function LiquidityCell({ liquidity }: { liquidity: Vault["liquidity"] }) {
  return (
    <TableCell>
        ...
        {Array.from({ length: 3 }).map((_, i) => (
          <div
            ...
            className={`h-1.5 w-3 rounded-full ${
              i < (liquidity === "high" ? 3 : liquidity === "medium" ? 2 : 1) ? "bg-primary" : "bg-muted"
            }`}
          />
        ))}
      </div>
